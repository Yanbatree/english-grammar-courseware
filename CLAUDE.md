...
### 中文小故事规范（学习目标页必须包含）
- 位置：学习目标页三个目标卡片下方，"三关通关"标语之后
- 样式：用 `.example-box` + `border-left-color:var(--blue)` 包裹
- 标签：`📖 小故事`
- 内容要求：
  - 用日常对话/校园场景，自然嵌入本课语法概念
  - 故事中每个关键语法点用对应颜色高亮（who→蓝、which→金、that→蓝等）
  - 故事结尾一句话点题，用 👉 引出核心结论
  - 让学生从"中文直觉"过渡到"英文语法规则"
- 示例：定语从句课1用"街上看到一个人→什么样的人？"，名词性从句用"小明考了第一"四个位置，状语从句用"周末经历"五种背景信息
